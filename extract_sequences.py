    ...
    for start_idx, start_block_tuple in enumerate(blocks):
        ...
        if quiet_period_before > 0: # Only check if a quiet period before is required
            if start_idx > 0:
                ...
                # This implies that if a sequence starts at frame 1, and quiet_period_before is 60,
                # it's not quiet before. Frame 0 is the implicit "previous event".
                # If quiet_period_before is 0, this condition current_start_frame <= 0 is usually false
                # unless frames can be 0 or negative.
                is_quiet_before = False
        ...
        # The actual start frame of the first behavior in this potential sequence
        sequence_actual_start_frame = current_start_frame
        last_behavior_end_frame_in_seq = current_end_frame
        # Keep track of the index of the last block successfully added to the sequence
        ...
        while current_block_scan_idx < num_blocks:
            ...
            # Check if this behavior is close enough to be part of the sequence
            if next_start_frame - last_behavior_end_frame_in_seq <= max_gap:
                current_sequence_ids.append(next_behavior_id)
                last_behavior_end_frame_in_seq = next_end_frame # Update the end frame of the *sequence*
                last_block_idx_in_sequence = current_block_scan_idx # Update index of the last block

                # Check if this fulfills the end_phase requirement (and we can potentially stop building)
                if end_phase is not None and next_behavior_id == end_phase:
                    break # Sequence potentially complete, exit inner loop to check other filters
            else:
                # Gap is too large, this behavior doesn't belong to the current sequence
                break # End sequence building

            current_block_scan_idx += 1

        # --- Sequence built (or consists of only start_block), now check filters ---

        # --- Filter 3: Check minimum length requirement (typically >= 2 for a "sequence") ---
        if len(current_sequence_ids) < 2:
             continue

        # --- Filter 4: Check end_phase requirement (if applicable) ---
        if end_phase is not None and current_sequence_ids[-1] != end_phase:
             continue

        # --- Filter 5: Check Max Sequence Duration ---
        # Duration is from the start of the first behavior to the end of the last identified behavior in the sequence
        ...
        if max_sequence_duration is not None and current_sequence_duration > max_sequence_duration:
            continue # Sequence is too long

        # --- Filter 6: Check for quiet period AFTER the sequence ---
        is_quiet_after = True
        if quiet_period_after > 0: # Only check if a quiet period after is required
            # End frame of the last block in the *identified* sequence
            seq_final_block_end_frame = blocks[last_block_idx_in_sequence][2]
            # Index of the next block *in the overall list of blocks*
            ...
            # else: If there are no more blocks, it's considered quiet after.

        # --- Final Decision: If all filters passed, add the sequence ---
        if is_quiet_after: # is_quiet_before was already checked
...
def process_file(file_path, max_gap=60,
    ...
    try:
        ...
        blocks = extract_behavior_blocks(df) # max_gap not used in extract_behavior_blocks
        if not blocks:
            return []
...
def process_group_files(directory, group_suffix, max_gap=60,
                        quiet_period_before=60, quiet_period_after=0,
                        max_sequence_duration=None,
                        start_phase=None, end_phase=None):
    """
    ...
    if not group_files:
         print(f"Warning: No files found for group suffix: {group_suffix} in directory {directory}")
         return []
    print(f"Found {len(group_files)} files for group: {group_suffix}")
    ...
    for file_path in group_files:
        try:
            sequences = process_file(file_path, max_gap,
                                     quiet_period_before, quiet_period_after,
                                     max_sequence_duration,
                                     start_phase, end_phase)
            if sequences:
                all_sequences.extend(sequences)
                print(f"Processed: {os.path.basename(file_path)}, found {len(sequences)} sequences.")
            files_processed_count += 1
        except Exception as e:
            print(f"Error processing file {file_path}: {str(e)}")
            files_error_count += 1

    print(f"Finished processing group {group_suffix}. Processed {files_processed_count} files, {files_error_count} encountered errors.")
...
def main():
    parser = argparse.ArgumentParser(
        description='Extract behavioral sequences from CSV files with independent quiet periods and max duration.',
    ...
    groups = ['_veh.csv', '_skf.csv', '_rop.csv'] # Consider making this configurable

    for group_suffix in groups:
        print(f"\n--- Processing Group: {group_suffix} ---")
        group_name = group_suffix.lower().replace('.csv', '').replace('_', '')

        sequences = process_group_files(args.directory, group_suffix, args.max_gap,
                                        args.quiet_before, args.quiet_after,
                                        args.max_seq_duration,
                                        start_phase_id, end_phase_id)

        if not sequences:
            print(f"No valid sequences for group: {group_suffix}. Skipping analysis/saving.")
            continue
        ...
        output_file_base = (f"sequences_{group_name}{phase_info}_gap{args.max_gap}"
